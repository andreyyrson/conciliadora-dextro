...
Com o `access_token` em mãos, as chamadas de extrato são diretas:

```typescript
// lib/cumbuca/client.ts

const CUMBUCA_PROXY = process.env.CUMBUCA_PROXY_BASE_URL
// ex: "https://proxy.cumbuca.com/{bank-id}/open-banking"

export const cumbuca = {

  // ── Consentimento ────────────────────────────────────────
  async criarConsentimento(clientToken: string, cpfOuCnpj: string) {
    const res = await fetch(`${CUMBUCA_PROXY}/consents/v3/consents`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${clientToken}`,
        'Content-Type': 'application/json', // simplificado para MVP; produção usa JWT
        'x-fapi-interaction-id': crypto.randomUUID(),
      },
      body: JSON.stringify({
        data: {
          loggedUser: { document: { identification: cpfOuCnpj, rel: cpfOuCnpj.length === 11 ? 'CPF' : 'CNPJ' } },
          permissions: ['ACCOUNTS_READ', 'ACCOUNTS_TRANSACTIONS_READ', 'ACCOUNTS_BALANCES_READ'],
          expirationDateTime: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString(),
        }
      })
    })
    return res.json() // { data: { consentId, status, links } }
  },

  // ── URL de autorização (redirecionar o usuário) ───────────
  buildAuthUrl(consentId: string, state: string, codeChallenge: string): string {
    const params = new URLSearchParams({
      response_type: 'code',
      client_id: process.env.CUMBUCA_CLIENT_ID!,
      scope: `openid accounts consent:${consentId}`,
      redirect_uri: process.env.CUMBUCA_REDIRECT_URI!,
      state,
      code_challenge: codeChallenge,
      code_challenge_method: 'S256',
    })
    return `${process.env.BANK_AUTH_URL}?${params}`
  },

  // ── Trocar código por token ───────────────────────────────
  async exchangeCode(code: string, codeVerifier: string): Promise<Tokens> {
    const res = await fetch(process.env.BANK_TOKEN_URL!, {
      method: 'POST',
  ...
  // ── Listar contas ─────────────────────────────────────────
  async getAccounts(accessToken: string): Promise<OFAccount[]> {
    const res = await fetch(`${CUMBUCA_PROXY}/accounts/v2/accounts`, {
      headers: {
    ...
    return json.data  // array de contas
  },
  ...
  async getTransactions(
    ...
    const params = new URLSearchParams({
      fromBookingDate: from.toISOString().split('T')[0],  // YYYY-MM-DD
      toBookingDate:   to.toISOString().split('T')[0],
    })
    const res = await fetch(
      `${CUMBUCA_PROXY}/accounts/v2/accounts/${accountId}/transactions?${params}`,
      { headers: { 'Authorization': `Bearer ${accessToken}`, 'x-fapi-interaction-id': crypto.randomUUID() } }
    )
    const json = await res.json()
    return json.data  // array de lançamentos
  },

  // ── Saldo ─────────────────────────────────────────────────
  async getBalance(accessToken: string, accountId: string) {
    const res = await fetch(
      `${CUMBUCA_PROXY}/accounts/v2/accounts/${accountId}/balances`,
      { headers: { 'Authorization': `Bearer ${accessToken}`, 'x-fapi-interaction-id': crypto.randomUUID() } }
    )
...
### Mapeamento Open Finance → Nosso Schema

O Open Finance retorna transações nesse formato — precisamos normalizar para nosso `ExtratoLancamento`:

```typescript
// lib/cumbuca/normalizer.ts

// Formato que o Open Finance Brasil retorna
interface OFTransaction {
  ...
  transactionName: string       // descrição
  type: string                  // PIX, TED, DOC, BOLETO, etc.
  amount: {
    amount: string              // "150.00" (string, cuidado!)
    currency: string            // "BRL"
  }
  transactionDate: string       // "2024-01-15"
  partieName?: string           // nome do pagador/recebedor
}

export function normalizarTransacao(t: OFTransaction, contaId: string) {
  return {
    contaId,
    identificador:  t.transactionId,
    data:           new Date(t.transactionDate),
    descricao:      `${t.transactionName}${t.partieName ? ` - ${t.partieName}` : ''}`,
    valor:          new Prisma.Decimal(t.amount.amount),  // nunca parseFloat de dinheiro!
    tipo:           t.creditDebitType,  // já vem 'CREDITO' | 'DEBITO'
  }
}
```

> ⚠️ **Atenção crítica:** O valor vem como `string` no Open Finance ("150.00"). **Nunca use `parseFloat()`** para dinheiro — use `new Prisma.Decimal(valor)` ou a biblioteca `decimal.js` diretamente.

---

### Variáveis de Ambiente Cumbuca

```bash
# Fornecidas pela Cumbuca no onboarding
CUMBUCA_PROXY_BASE_URL="https://proxy.cumbuca.com/{bank-id}/open-banking"
CUMBUCA_CLIENT_ID="seu-client-id"
CUMBUCA_CLIENT_SECRET="seu-client-secret"
CUMBUCA_REDIRECT_URI="https://seusite.com/api/auth/callback/openfinance"

# Endpoints do banco específico (variam por instituição)
# A Cumbuca fornece um diretório com todos os bancos participantes
BANK_AUTH_URL="https://auth.banco.com.br/as/authorization.oauth2"
BANK_TOKEN_URL="https://auth.banco.com.br/as/token.oauth2"
```

> Para o **MVP**, comece com um único banco (ex: Itaú ou Bradesco — os maiores) e expanda depois. A Cumbuca fornece o diretório completo dos bancos participantes e os endpoints de cada um.

---

### Callback Route Handler (Next.js)

```typescript
// app/api/auth/callback/openfinance/route.ts
...
export async function GET(req: Request) {
  ...
  // Recupera state da sessão (armazenado no POST /api/contas)
  const { contaId, codeVerifier } = await getStateFromSession(state)

  // Troca código por token
  const tokens = await cumbuca.exchangeCode(code, codeVerifier)

  // Salva tokens na ContaBancaria
  await prisma.contaBancaria.update({
    where: { id: contaId },
    data: {
      accessToken:   tokens.access_token,
      refreshToken:  tokens.refresh_token,
      tokenExpiraEm: new Date(Date.now() + tokens.expires_in * 1000),
    }
  })

  return NextResponse.redirect('/contas?conectado=true')
}
```
