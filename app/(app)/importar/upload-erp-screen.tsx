...
export function UploadErpScreen() {
  ...
  const [extracaoPreview, setExtracaoPreview] = useState<{ totalLinhas: number; preview: { data: string; valor: number; tipo: string; descricao: string; fornecedor: string | null; banco: string | null; categoria: string | null }[] } | null>(null)
  ...
  const handleAnalisar = async (e: React.FormEvent) => {
    ...
    if (!file || !empresaId || !date) {
      setError("Selecione uma empresa, o arquivo e o período")
      setLoading(false)
      return
    }
    ...
    reader.onload = async (e) => {
      ...
      let previewLines: Record<string, unknown>[] = []
      ...
      if (file.name.endsWith('.csv')) {
        ...
        previewLines = (result.data as Record<string, unknown>[]).slice(0, 100)
      } else if (file.name.endsWith('.xlsx')) {
        const workbook = XLSX.read(content, { type: 'binary' })
        const sheetName = workbook.SheetNames[0]
        const worksheet = workbook.Sheets[sheetName]
        const jsonData = XLSX.utils.sheet_to_json(worksheet)
        previewLines = (jsonData as Record<string, unknown>[]).slice(0, 100)
      }
    ...
    return (
      <Card className="p-6">
        <p className="text-muted-foreground">
          Selecione uma empresa no topo da página para fazer upload de arquivos do ERP.
        </p>
      </Card>
    )
  }
