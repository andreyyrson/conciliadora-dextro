...
import { revisarBody } from "@/lib/api-schemas"
...
export async function POST(req: Request) {
  try {
    ...
    const parsed = revisarBody.safeParse(body)
    if (!parsed.success) {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const atual = await tx.aprovacaoDia.upsert({
        where: { empresaId_dataDia_banco: { empresaId, dataDia: dia, banco } },
        update: { status: "A_REVISAR", justificativa: justificativa || null, userId },
        create: { empresaId, dataDia: dia, banco, status: "A_REVISAR", justificativa: justificativa || null, userId },
      })

      await tx.aprovacaoDiaLog.create({
        data: { aprovacaoDiaId: atual.id, deStatus, paraStatus: "A_REVISAR", justificativa: justificativa || null, userId }
      })
      return atual
    })
    ...
    return NextResponse.json({ error: e.message || "Erro ao marcar dia para revisão" }, { status: 500 })
  }
}
