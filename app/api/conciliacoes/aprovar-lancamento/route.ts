...
import { aprovarLancamentoBody } from "@/lib/api-schemas"
...
export async function POST(req: Request) {
  try {
    ...
    const parsed = aprovarLancamentoBody.safeParse(body)
    if (!parsed.success) {
    ...
    const result = await prisma.aprovacaoLancamento.upsert({
      ...
      create: { empresaId, dataDia: dia, extratoId, status: "APROVADO", justificativa: justificativa || null, userId },
    })

    return NextResponse.json({ status: result.status, dataDia, extratoId, updatedAt: result.updatedAt }, { status: 200 })
  } catch (e: any) {
    return NextResponse.json({ error: e.message || "Erro ao aprovar lançamento" }, { status: 500 })
  }
}
